<template>
    <div class="my-8 grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-8 md:gap-16">
        <div v-if="article">
            <img :src="article.image" alt="" class="w-full h-72 object-cover rounded-lg mb-8">
            <h1 class="text-xl md:text-2xl lg:text-3xl font-bold mb-4">{{ article.title }}</h1>
        ...
        <div v-if="articlesFromTheSameCategory">
            <h2 class="text-lg md:text-xl lg:text-2xl font-bold mb-4">Articles from the same category</h2>
...
onMounted(async () => {
    try {
        ...
        articlesFromTheSameCategory.value = await $fetch(`/api/posts/category/${categoriesTypes}`);
    } catch (error) {
